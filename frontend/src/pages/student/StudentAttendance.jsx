...
import { calendarData, courseBreakdownData, attendanceHistoryData } from '../../mock/attendanceData';
                ...
                <div className="max-w-7xl mx-auto space-y-8">
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <div className="bg-white p-6 rounded-2xl shadow-card hover:shadow-card-hover transition-all border border-gray-100/50">
                            ...
                            <h3 className="text-4xl font-extrabold text-primary">92.4%</h3>
                        ...
                        <div className="bg-white p-6 rounded-2xl shadow-card hover:shadow-card-hover transition-all border border-gray-100/50">
                            ...
                            <h3 className="text-4xl font-extrabold text-primary">148</h3>
                            <p className="mt-4 text-gray-400 text-xs font-medium">Out of 160 total lectures</p>
                        </div>
                        <div className="bg-white p-6 rounded-2xl shadow-card hover:shadow-card-hover transition-all border border-gray-100/50">
                            ...
                            <p className="mt-4 text-red-400 text-xs font-medium">3 unexplained absences</p>
                        </div>
                    </div>
                            ...
                            <div className="p-8">
                                <div className="grid grid-cols-7 gap-y-8 text-center">
                                    ...
                                    {calendarData.dates.map((date, index) => {
                                        const statusColor = date.status === 'present' ? 'bg-green-500' : 
                                                          date.status === 'absent' ? 'bg-red-500' : 
                                                          date.status === 'no-class' ? 'bg-gray-300' : '';
                                        
                                        return (
                                            <div key={index} className={`h-16 flex flex-col items-center justify-center group cursor-pointer relative ${date.today ? 'bg-primary/5 rounded-2xl' : ''}`}>
                                                <span className={`text-sm ${date.today ? 'font-bold text-primary' : date.currentMonth ? 'font-bold text-gray-800' : 'font-medium text-gray-300'}`}>
                                                    {date.day}
                                                </span>
                                                {date.status && (
                        ...
                        <div className="lg:col-span-4 space-y-6">
                            <div className="bg-white rounded-2xl shadow-card p-6 border border-gray-100/50">
                                ...
                                <div className="space-y-6">
                                    {courseBreakdownData.map(course => (
                                        <div key={course.name}>
                                            ...
                                            <p className="mt-2 text-[10px] text-gray-400 font-medium">Next: {course.next}</p>
                                        </div>
                                    ))}
                                </div>
                            </div>
                        ...
                        <div className="overflow-x-auto">
                            <table className="w-full text-left">
                                ...
                                <tbody className="divide-y divide-gray-50">
                                    {attendanceHistoryData.map((row, i) => (
                                        <tr key={i} className="hover:bg-gray-50 transition-colors">
                                            <td className="px-6 py-4">
                                                <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-bold bg-${row.color}-50 text-${row.color}-600`}>
                                                    <span className={`w-1.5 h-1.5 rounded-full bg-${row.color}-500`}></span>
                                                    {row.status}
                                                </span>
                                            </td>
                                            <td className="px-6 py-4 text-sm font-semibold text-gray-800">{row.course}</td>
                                            <td className="px-6 py-4 text-sm text-gray-500">{row.date}</td>
                                            <td className="px-6 py-4 text-sm text-gray-500">{row.duration}</td>
                                            <td className="px-6 py-4 text-sm text-gray-400">{row.remarks}</td>
                                        </tr>
                                    ))}
                                </tbody>
                            </table>
